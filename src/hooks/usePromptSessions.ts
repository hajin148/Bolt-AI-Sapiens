...
export const usePromptSessions = () => {
  ...
  const updateSession = async (id: string, updates: Partial<CreatePromptSessionData & { main_prompt?: string; is_favorited?: boolean }>) => {
...
export const usePromptMessages = (sessionId: string) => {
  ...
  const addMessage = async (messageData: Omit<CreatePromptMessageData, 'session_id'>) => {
    const { data, error } = await supabase
      .from('prompt_messages')
      .insert({
